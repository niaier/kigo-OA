    ...
    <div class="properties-body">
      ...
      <a-form v-show="selectItem.key !== ''">
        ...
        >
          ...
          />
          <a-select
            v-if="tableType == '1' && selectItem.type != 'batch'"
            v-model="selectItem.model"
            placeholder="请选择数据字段"
        ...
        <a-form-item
          v-if="selectItem.type === 'select' || selectItem.type === 'radio'"
          label="关联属性"
        >
          <KChangeOption
            v-model="selectItem.linkFieldSet"
            type="linkFieldSet"
        ...
        <a-form-item
          v-if="
            typeof options.option_type !== 'undefined' &&
            selectItem.type === 'loginUserOrganize'
          "
          label="选择-当前用户或组织"
        >
          <a-select default-value="0" v-model="options.option_type">
            <a-select-option value="0"> 当前用户 </a-select-option>
            <a-select-option value="1"> 当前组织 </a-select-option>
          </a-select>
        </a-form-item>
        ...
        <a-form-item
          v-if="
            typeof options.option_type !== 'undefined' &&
            selectItem.type === 'selectUserOrganize'
          "
          label="选择-当前用户或组织"
        >
          <a-select default-value="0" v-model="options.option_type">
            <a-select-option value="0"> 选择单用户 </a-select-option>
            <a-select-option value="1"> 选择单组织 </a-select-option>
            <a-select-option value="2"> 选择多用户 </a-select-option>
            <a-select-option value="3"> 选择多组织 </a-select-option>
          </a-select>
        </a-form-item>
...
import { getTableList, getColumndList } from "@/api/customize/vform";
...
export default {
  ...
  computed: {
    options() {
      this.selectChange(this.selectItem);
      return this.selectItem.options || {};
    },
    tableType() {
      return store.getters.tableType;
    },
  },
  asyncComputed: {
    fieldData() {
      if (this.tableType == "1") {
        this.findFieldData(store.getters.tableName);
      }

      /**return getColumndList({ table_name: store.getters.tableName }).then(
        (response) => {
          let data = response.data.data;
          if (this.tableType == "1") {
            let resData = data.filter((item) => {
              return item.column_name == this.selectItem.model;
            });
            if (resData.length == 0) {
  ...
  methods: {
    selectChange(record) {
      if (this.tableType == "1" && this.selectItem.type != "batch") {
        //处理batch下的数据
        if (this.selectItem.table_id != undefined) {
          let tableName = this.selectItem.table_id;
          this.findFieldData(tableName);
        } else {
          let tableName = store.getters.tableName;
          this.findFieldData(tableName);
        }
      }
      if (this.tableType == "1" && this.selectItem.type == "batch") {
        this.modelTitle = "选择数据子表";
        let tableList = this.formData.filter(
          (item) => item.table_name == this.selectItem.model
        );
        if (tableList.length == 0) {
          this.selectItem.model = "";
        }
      } else if (this.tableType == "0" && this.selectItem.type == "batch") {
        this.modelTitle = "请输入数据子表";
      } else {
        this.modelTitle = "数据字段";
      }
    },
    ...
    selectFieldChange(value, option) {
      let label = option.data.attrs.text;
      if (label != undefined) {
        this.selectItem.label = label;
      }
    },
    findFieldData(table_name) {
      getColumndList({ table_name: table_name }).then((response) => {
        this.fileData = response.data.data;
        if (this.tableType == "1") {
          let resData = this.fileData.filter((item) => {
            return item.column_name == this.selectItem.model;
          });
          if (resData.length == 0) {
            this.selectItem.model = "";
    ...
    findDictionary() {
      ...
      findDictionaryList({
        parent_code: this.selectItem.options.dynamicKey,
      }).then((response) => {
        ...
        console.log(options);
        this.selectItem.options.options = options;
      });
    },
  },
};
