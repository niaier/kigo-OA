...
import { getListPage, delData, updateStatus } from "@/api/system/user";
...
export default {
  ...
  methods: {
    ...
    fetch(params = {}) {
      this.loading = true;
      getListPage({ organize_id: this.organize_id, ...params }).then(
        (response) => {
    ...
    download() {
      window.location.href =
        ...
        "&organize_id="+this.organize_id+"&authName="+this.authName;
    },
    dialogData(record) {
      const that = this;
