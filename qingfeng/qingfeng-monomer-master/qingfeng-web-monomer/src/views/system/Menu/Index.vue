    ...
    <a-row :gutter="24">
      <a-col :md="24" :lg="6">
        <a-card :bordered="false">
          <dic-tree ref="child" @selectTree="selectTree"></dic-tree>
        </a-card>
      </a-col>
      <a-col :md="24" :lg="18">
        <a-card :bordered="false">
          ...
          <div style="margin: 0 0 10px 0">
            <a-space size="small">
              <a-button v-action:menu:add @click="add()">添加</a-button>
              <a-button
                v-action:menu:edit
                type="primary"
                @click="edits()"
              ...
              <div class="editable-row-operations">
                <a-space size="small">
                  ...
                  <a-button
                    size="small"
                    type="primary"
                    v-action-dauth:[{...record,auth:`menu:edit`}]
                    @click="() => edit(record)"
                    >编辑</a-button
                  >
                  <a-button
                    size="small"
                    type="danger"
                    v-action-dauth:[{...record,auth:`menu:del`}]
                    @click="() => del(record)"
                    >删除</a-button
                  >
                  <a-button
...
import { geiListPage, delData, updateStatus } from "@/api/system/menu";
...
export default {
  ...
  methods: {
    ...
    selectTree(id, name, value) {
      ...
      this.menu_cascade = value.split(":")[0];
      this.level_num = value.split(":")[1];
      this.parent_type = value.split(":")[2];
      this.fetch({ pageSize: 10 });
    },
    fetch(params = {}) {
      this.loading = true;
      geiListPage({ parent_id: this.parent_id, ...params }).then((response) => {
    ...
    dialogData(record) {
      ...
      record = {
        ...
        menu_cascade: this.menu_cascade,
    ...
    getTreeList() {
      this.$refs.child.getTreeList({ pid: this.parent_id });
    },
  },
};
</script>
