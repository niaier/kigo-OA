...
export default {
  ...
  mounted() {
    // this.getTreeList({type:this.type});
  },
  methods: {
    onSelect(keys, info) {
    ...
    getTreeList(params) {
      findList({...params}).then((response) => {
        ...
        this.$emit(
          ...
          treeData[0].id,
          treeData[0].name,
          treeData[0].dc_cascade + ":" + treeData[0].level_num
        );
      });
    },
    fommat({
      ...
      arrayList.push({'name':'分类信息','id':'1','parent_id':'0','dc_cascade':'dc_1_','level_num':'0'})
      ...
      for (let i = 0; i < arrayList.length; i++) {
        ...
        data.key = data.id;
        data.value = data.dc_cascade + ":" + data.level_num;
        if (data.child_num == 0) {
          data.isLeaf = true;
        }
