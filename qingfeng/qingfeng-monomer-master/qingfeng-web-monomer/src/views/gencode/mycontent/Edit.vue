<template>
  ...
  >
    <a-form-model-item label="标题" prop="title">
      <a-input v-model="form.title" type="textarea" />
    </a-form-model-item>
    <a-form-model-item ref="intro" label="简介" prop="intro">
      ...
      <div
        v-for="(item, index) in introFileList"
         :key="index"
        :style="{ lineHeight: '30px' }"
      >
        <a @click="downloadintroFile(item)"
          ><a-icon type="link" />{{ item.name }}</a
        >
        <a
          @click="delintroFile(item)"
    ...
    <a-form-model-item ref="content" label="内容" prop="content">
      <a-select v-model="form.content" placeholder="请选择内容">
        <a-select-option v-for="(item, index) in content_data" :key="index" :value="item.id">
          {{ item.name }}
        </a-select-option>
      </a-select>
    </a-form-model-item>
    <a-form-model-item ref="read_num" label="阅读数量" prop="read_num">
      <a-checkbox-group v-model="form.read_num">
        <a-checkbox v-for="(item, index) in read_num_data" :key="index" :value="item.id">{{
          item.name
        }}</a-checkbox>
      </a-checkbox-group>
    </a-form-model-item>
    ...
    <a-form-model-item ref="remark" label="备注" prop="remark">
      <rich-text :text="form.remark" @editorChange="editorChange" />
    </a-form-model-item>
  </a-form-model>
</template>
<script>
  ...
  methods: {
    ...
    uploadintroFile(file) {
      ...
      upload(formData).then(
        (res) => {
          if (res.status == 200) {
            this.introFileList.push(res.data.data);
            this.uploading = false;
            this.form.intro = this.introFileList
              .map((map) => {
                return map.id;
              })
              .join(",");
    ...
    onOk() {
      return new Promise((resolve) => {
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            this.form.read_num = this.form.read_num.join(",");
