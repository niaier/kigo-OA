<template>
    ...
    >
                ...
                <a-form-model-item ref="intro" label="简介" prop="intro">
                    <div v-for="(item, index) in introFileList" :key="index" :style="{ lineHeight: '30px' }">
                ...
                <a-form-model-item ref="content" label="内容" prop="content">
                    <a-select disabled v-model="form.content" placeholder="请选择内容">
                        <a-select-option v-for="(item, index) in content_data" :key="index" :value="item.id"> {{item.name}} </a-select-option>
                    </a-select>
                </a-form-model-item>
                <a-form-model-item ref="read_num" label="阅读数量" prop="read_num">
                    <a-checkbox-group disabled v-model="form.read_num">
                        <a-checkbox v-for="(item, index) in read_num_data" :key="index" :value="item.id">{{item.name}}</a-checkbox>
                    </a-checkbox-group>
                </a-form-model-item>
