<template>
  <div style="margin-top: 16px">
    ...
    <el-form-item label="处理用户" v-show="userTaskForm.type == '0'">
      <el-input
        v-model="userTaskForm.assigneeName"
        clearable
...
export default {
  ...
  methods: {
    resetTaskForm() {
      ...
      findAssignment({
        ...
        this.$set(
          this.userTaskForm,
          "assign_mode",
          response.data.data.assign_mode
        );
        this.$set(
          this.userTaskForm,
          "assign_content",
          response.data.data.assign_content
        );
        ...
        if (response.data.data.assign_mode == "1") {
          this.$set(
            this.userTaskForm,
            "organize_id",
            response.data.data.organize_id
          );
          this.$set(
            this.userTaskForm,
            "organize_name",
            response.data.data.organize_name
          );
        } else if (
          response.data.data.assign_mode == "8" ||
          response.data.data.assign_mode == "10"
        ) {
          this.$set(this.userTaskForm, "user_ids", response.data.data.user_ids);
          this.$set(
            this.userTaskForm,
            "user_names",
            response.data.data.user_names
          );
          ...
          this.$set(
            this.userTaskForm,
            "organize_ids",
            response.data.data.organize_ids
          );
          this.$set(
            this.userTaskForm,
            "organize_names",
            response.data.data.organize_names
          );
        }
      });
    },
    ...
    setAssignment() {
      //更新文档参数
      this.updateActivitiProperties("assignee", this.userTaskForm.assigneeName);
    },

    setCandidateUsers() {
      //更新文档参数
      this.updateActivitiProperties(
        "candidateUsers",
        this.userTaskForm.candidateUsersName
      );
    },
    ...
    setCandidateGroups() {
      ...
      this.updateActivitiProperties(
        "candidateGroups",
        this.userTaskForm.candidateGroupsName
      );
    },
