...
<script>
import { saveOrUpdate } from "@/api/activiti/bpmn";
// 生产环境时优化
...
import { deployByString } from "@/api/activiti/modeler";
...
export default {
  ...
  methods: {
    ...
    async downloadProcess(type, name) {
      try {
        ...
        if (type === "xml" || type === "bpmn") {
          ...
          const { err, xml } = await this.bpmnModeler.saveXML();
          deployByString({ xmlString: xml }).then((response) => {
            console.log(response);
          });
          // axios.post("/rest/mng/activiti/applyer/addDeploymentByString",{
          //   "xmlString":xml
          // }).then(res=>{
          //   console.log(res)
          // })
    ...
    async save() {
      ...
      let j = convert.xml2js(xml);
      let versionTag =
        j.elements[0].elements[0].attributes["activiti:versionTag"];
      ...
      saveOrUpdate({
        xmlString: xml,
        type: "0",
        id: j.elements[0].elements[0].attributes.id,
        processId: j.elements[0].elements[0].attributes.id,
        processName: j.elements[0].elements[0].attributes.name,
        versionTag: versionTag,
        isExecutable: j.elements[0].elements[0].attributes.isExecutable,
      }).then((response) => {
    ...
    async saveAndDeploy() {
      ...
      saveOrUpdate({
        ...
        if (response.data.success) {
          this.$message.success("保存成功且发布成功。");
        }
      });
    },
