    ...
    <a-row v-if="type === 'linkFieldSet'" :gutter="8">
      <span v-for="(val, index) in value" :key="index">
        <div class="option-change-box">
          ...
          <a-col :span="24"
            ><a-input
              v-model="val.showLinkField"
              placeholder="显示(数据字段，逗号分割)"
            ></a-input
          ></a-col>
          <a-col :span="24"
            ><a-input
              v-model="val.hideLinkField"
              placeholder="隐藏(数据字段，逗号分割)"
            ></a-input>
          </a-col>
        </div>
        <a-divider style="border-color: #13c2c2; margin: 4px 0" dashed />
      </span>

      <a-col :span="24"><a @click="handleAddLinkField">增加</a></a-col>
    </a-row>
...
export default {
  ...
  mounted() {
    // if(this.value==undefined&&this.type=='linkFieldSet'){
    //   this.value = [{
    //       linkField: "",
    //       showLinkField: "",
    //       hideLinkField: "",
