...
import com.qingfeng.system.mapper.GentableMapper;
            ...
            for (PageData param:list) {
                ...
                if(fieldList.size()>0){
                    for (PageData fieldParam:fieldList) {
                        ...
                        genfield.setField_comment(fieldParam.get("column_comment").toString());
                        genfield.setField_type(fieldParam.get("data_type").toString());

                        if(serverSystemProperties.getGencodeField().contains(fieldParam.get("column_name").toString())){
