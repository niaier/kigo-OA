...
//        System.out.println(SecurityContextHolder.getContext().getAuthentication().getName());
    ...
    public void deleteUsers(@NotBlank(message = "{required}") @PathVariable String userIds,HttpServletResponse response) throws Exception {
        Json json = new Json();
        try {
    ...
    public MyResponse findLoginUser() {
        ...
        //处理图片
        QueryWrapper queryWrapper = new QueryWrapper();
        queryWrapper.eq("obj_id",p.get("id"));
        ...
        if(fileList.size()>0){
            ...
            p.put("imageUrl", ParaUtil.cloudfile+uploadFile.getFile_path());
        }else{
            p.put("imageUrl", "");
        }
        //查询用户组织
    ...
    public MyResponse updateSwitchOrganize(@RequestBody PageData pd) throws MyException {
        Json json = new Json();
        try {
            String userParams = SecurityContextHolder.getContext().getAuthentication().getName();
            //处理数据权限
