...
import com.qingfeng.auth.manager.UserManager;
        ...
        if(Verify.verifyIsNotNull(uPd)){
            if(uPd.getString("status").equals("2")){
                ...
                if(uPd.getString("status").equals("0")){
                    ...
                    grantedAuthorityList.add(new SimpleGrantedAuthority("ROLE_ACTIVITI_USER"));
                     //在每个user中都添加ROLE_ACTIVITI_USER角色 在做到拾取和取消拾取任务时发现需要强绑定ACTIVITI角色，不指定角色的话报错UsernameNotFoundExceptio
