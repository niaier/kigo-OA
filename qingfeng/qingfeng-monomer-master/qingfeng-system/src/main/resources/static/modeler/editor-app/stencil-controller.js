                ...
                if (shapes && shapes.length > 0) {
                    ...
                    for (var i = 0; i < properties.length; i++) {
                        ...
                        if (propertyConfig === undefined || propertyConfig === null) {
                            ...
                            //自定义处理
                            var val = selectedShape.properties[key];
                            if (stencil.id().indexOf('StartNoneEvent') !== -1){
                                if(property.id()=='overrideid'){
                                    if(val!='SN_start_node'){
                                        val = "SN_start_node";
                                    }
                                    $rootScope.nodeKey = val;
                                }
                            }else if (stencil.id().indexOf('UserTask') !== -1){
                                if(property.id()=='overrideid'){
                                    console.log('----------------');
                                    console.log(val)
                                    if(val.indexOf('UT')==-1){
                            ...
                            if(property.id()=='processformfield'){
                            ...
                            if (stencil.id().indexOf('StartNoneEvent') !== -1){
                                ...
                                if(property.id()=='overrideid'){
                                    ...
                                    $http.get(ACTIVITI.CONFIG.activitiRoot+"/activiti/assignment/findActivitiAssignment?node_key="+val+"&model_id="+$rootScope.modelData.modelId).success(function(response){
                                        if(response.data!=''&&response.data!=null&&response.data!=undefined){
                                            currentProperty.value = {};
                                            currentProperty.value.assign_type = response.data.type;
                                            currentProperty.value.assign_mode = response.data.assign_mode;
                                            $scope.updatePropertyInModel(currentProperty);
                                            console.log("-----------------------------");
                                            console.log(currentProperty);
                                        }
        ...
        $scope.updatePropertyInModel = function (property, shapeId) {
            ...
            if (newValue != oldValue) {
                var commandClass = ORYX.Core.Command.extend({
                    ...
                    execute: function () {
                        //console.log("进来了，，，"+this.key+"#"+this.newValue);
